...
# Students should implement the missing parts indicated by "# Implement me"
...
layout( matrix(c(1,1,1,2,3,4),nrow=3,byrow=FALSE) , widths=c(1,2) )
plotPost( m , breaks=seq(0.9,3.1,0.2) , cenTend="mean" , xlab="m" , main="Model Index" )
plotPost( thetaM1 ,
...
plotPost( thetaM2 ,
          ...
          cex.main=1.75 , xlab=bquote(theta) , xlim=c(0,1) )
plotPost( thetaM3 ,
          main=bquote( theta*" when m=3" * " ; p(m=3|D)" == .(signif(pM3,3)) ) ,
          cex.main=1.75 , xlab=bquote(theta) , xlim=c(0,1) )
